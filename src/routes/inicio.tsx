...
function InicioPage() {
  ...
  const accountsTotal = accounts?.reduce((acc, curr) => acc + (curr?.balance ?? 0), 0) ?? 0;
  ...
  const totalGoal = matchingGoals.reduce((s, g) => s + (g?.amount ?? 0), 0);
  ...
  useEffect(() => {
    ...
    const daysBefore = alerts?.daysBefore ?? 7;
    const today = new Date();
    const limit = new Date();
    limit.setDate(today.getDate() + daysBefore);
    ...
    upcoming.slice(0, 3).forEach((b) => {
      ...
      toast.warning(`Vence em breve: ${b.item}`, {
        description: `${BRL(b.amount ?? 0)} • ${due.toLocaleDateString("pt-BR")}`,
      });
    });
  }, [fixedBills, alerts]);
  ...
  const byCategoryTotal = byCategory.reduce((s, c) => s + (c?.value ?? 0), 0);
  const chartData = byCategory.length > 0 ? byCategory : [];
            ...
            <div className="mt-3 flex flex-col items-center gap-4 sm:flex-row sm:items-center sm:gap-6">
              <div className="h-36 w-36 shrink-0 sm:h-40 sm:w-40">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={chartData} dataKey="value" innerRadius={42} outerRadius={70} paddingAngle={2} stroke="none">
                      {chartData.map((c) => (
                        <Cell key={c.name} fill={categoryColors[c.name] ?? "oklch(0.5 0.02 250)"} />
                      ))}
                    </Pie>
                    <ReTooltip
                      formatter={(value: number, name: string) => {
                        const pct = byCategoryTotal > 0 ? Math.round((value / byCategoryTotal) * 100) : 0;
                        return [`${BRL(value ?? 0)} (${pct}%)`, name];
                      }}
                ...
                {chartData.map((c) => (
                  <li key={c.name} className="flex items-center gap-2">
                    <span
                      ...
                      style={{ background: categoryColors[c.name] ?? "oklch(0.5 0.02 250)" }}
                    />
                    <span className="min-w-0 flex-1 truncate text-muted-foreground">{c.name}</span>
                    <span className="shrink-0 font-medium text-foreground">{BRL(c.value ?? 0)}</span>
                  </li>
                ))}
              </ul>
            </div>
